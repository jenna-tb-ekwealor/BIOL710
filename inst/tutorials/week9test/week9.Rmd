...
To build intuition, we can:
- Visualize how far individual values deviate from their group mean vs. the grand mean.
- Plot model residuals — the differences between observed values and their predicted group mean — to see what's *not* explained.

One way to visualize this is to plot the **group means** and overlay the **grand mean** (mean of all observations), to see how much of the variation is due to differences among group means.
...
ggplot(plant_growth, aes(x = group, y = weight)) +
  ...
  labs(title = "Group Means and Grand Mean", subtitle = "Red dashed line = grand mean")
```
