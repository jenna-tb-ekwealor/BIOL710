...
Fill in the missing code to visualize the boxplot, grand mean, and ANOVA p-value.

To better understand the results of the ANOVA, it's useful to visualize how group means compare to the overall (grand) mean, and whether the p-value suggests statistically significant differences.
...
```{r rsq-plot-hint, exercise = TRUE}
# Hint: use mean() and reference the weight column inside plant_growth
grand_mean <- ___(plant_growth$___)

# For the plot, use ggplot(), set aes(x = ___, y = ___), and use geom_boxplot()
``` 
