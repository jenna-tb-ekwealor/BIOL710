...
plants <- read.csv("plants.csv", header = T)
```
...
ggplot(data = plants, aes(x = day, y = females)) + 
  geom_point()
```
...
ggplot(data = plants, aes(x =day, y = males)) + 
  geom_point()
```
...
**A note on piping:** Note that we can read these two lines of code as the following: *"Take the `plants` dataset and **pipe** it into the `mutate` function. 
Mutate the `plants` data set by creating a new variable called `total` that is the sum of the variables called `males` and `females`. 

Then assign the resulting dataset to the object called `plants`, i.e. overwrite the old `plants` dataset with the new one containing the new variable."*
...
In this case, you already have an object called `plants`, so this command updates that data set with the new mutated column.
...
ggplot(data = plants, aes(x = day, y = total)) + 
  geom_line()
```
...
plants <- plants %>% mutate(total = males + females) %>%
...
ggplot(data = plant, aes(x =day, y = male_ratio)) + 
  geom_line()
```
...
This command adds a new variable to the `plant` dataframe containing the values of either `TRUE` if that day had more males than females, or `FALSE` if that day did not (the answer may surprise you). 
This variable contains a different kind of data than we have encountered so far. 
All other columns in the `plant` data frame have values that are numerical (the day, the number of males and females). 
...
In the previous few exercises, you recreated some of the displays and preliminary analysis of historical spring flower data. 
Your assignment involves repeating these steps, but for present day germination records in the United States. 
...
Here's an example of how to find the minimum and maximum amount of male germinations in a day for the `plant` data:
...
What days are included in the `present` data set? 
...
present %>%
  summarise(min = min(day), max = max(day))
```
...
In what day did we see the most total number of germinations in the U.S.? *Hint:*
