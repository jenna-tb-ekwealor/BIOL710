---
title: "Introduction to spaial data in R"
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
```{r tmap-points2-setup, message=FALSE}
# to avoid issues with interactive maps https://github.com/rstudio/learnr/issues/529
# IdSeed must be set to be unique in the Rmd, even for mode('plot') to allow user to change
tmap_mode("plot")
htmlwidgets::setWidgetIdSeed(1)
...
Plot the map with point color scaled to population of each city.  
```{r tmap-points2, exercise = T}
tm_shape(___) + 
  tm_symbols(col = ___)  
```

```{r tmap-points2-hint}
colnames(africapitals) # to view column names
```

```{r tmap-points2-solution}
tm_shape(africapitals) + 
  tm_symbols(col = "pop")  
```

```{r tmap-points2-check}
# check code
