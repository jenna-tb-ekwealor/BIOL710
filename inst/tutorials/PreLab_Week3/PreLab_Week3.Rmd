...
mints <- pollen %>%
  filter(plant_group == "mint")
daisies <- pollen %>%
  filter(plant_group == "daisy")
...
515 species from different plant families.
...
You'll be focusing on just three columns to get started: plant_group, calories, 
grams of protein.

Let's first focus on just products from mints and daisies.
...
from protein of the options from these two plant_groups.  How do their centers, shapes,
and spreads compare?
...
You'll be focusing on grams of protein from daisies products, so let's store
...
The plot for daisies's grams of protein shows points that tend to follow the
...
specify to be the same number of menu items in the `daisies` data set using the 
...
Queen product has more than 600 grams of protein?"

If we assume that the grams of protein from daisies's menu are normally 
...
```{r pnorm, exercise = T}
1 - pnorm(q = 600, mean = daisymean, sd = daisysd)
```
...
interested in the probability that a daisy has more than 600 calories
...
2. In the plot above does the normal probability plot for the grams of protein in daisies items look similar to the plots created for the simulated data?  That is, do the plots provide evidence that the grams of protein are nearly normal? Explain.

3. Write out a probability question that you would like to answer about any 
    of the plant_groups in this dataset that would take advantage of `pnorm`.  Calculate that probability.

4.  Write out a question that you would like to answer about any 
    of the plant_groups in this dataset that would take advantage of `qnorm`.  Calculate that quantile.
